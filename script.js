...
function initializeNavigation() {
    ...
    if (viewWorkoutsCard) {
        viewWorkoutsCard. addEventListener('click', function() {
            window.location.href = 'gym-exercise-log/views/workouts.php';
        });
        
        // Keyboard navigation
        viewWorkoutsCard.setAttribute('tabindex', '0');
        viewWorkoutsCard.addEventListener('keypress', function(e) {
    ...
    if (addWorkoutCard) {
        addWorkoutCard. addEventListener('click', function() {
            window.location.href = 'gym-exercise-log/views/add_workout.php';
        });
        
        // Keyboard navigation
        addWorkoutCard.setAttribute('tabindex', '0');
        addWorkoutCard. addEventListener('keypress', function(e) {
            if (e. key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                window.location.href = 'gym-exercise-log/views/add_workout.php';
            }
